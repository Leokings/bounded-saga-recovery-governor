...
Current-source v0.2.1 exact deployment: **PASS**. Contract `0xA2DDebc4CC8Eb21bb8eB45214Bfad1A4dE7A26Fd` finalized with five `AGREE` votes and exact byte-for-byte equality to the 37,980-byte source at SHA-256 `00050b640db0c2c944fdd7aeb2d70c1715eedd635272478314fa74ec0c9209a4`.

Bradbury semantic smoke: **PASS on the explicit-evidence workflow**. Three attempts for the original reference remain disclosed: `VALIDATORS_TIMEOUT`, `NO_MAJORITY`, and finalized `DISAGREE` (one `AGREE`, three `DETERMINISTIC_VIOLATION`, one `TIMEOUT`); none persisted a decision. A new reference then supplied explicit authenticated evidence that no entitlement existed and that the captured payment remained fully refundable with no refund issued. Its four setup writes each finalized with five `AGREE` votes. Governance transaction `0xe7f59e31809bc3db775b768a83f0e0eba42477a170b2b7ab111fc738f66d30b7` finalized with four `AGREE` and one `DETERMINISTIC_VIOLATION`, and successful execution.

Latest-final reads after the successful workflow returned `DECIDED`, decision count `1`, and `PLAN_SELECTED / UNIQUE_RECOVERY_MATCH / REFUND_CAPTURED_PAYMENT`; the stored decision, request, reports, summary, and configuration each have recorded digests. Source readback remained byte-identical. Public receipts do not expose every validator's private provider log, so the proof makes no model-identity or per-validator LLM-call claim. See [`deployments/bradbury-2026-08-12-v0.2.1-deployment-and-smoke.json`](deployments/bradbury-2026-08-12-v0.2.1-deployment-and-smoke.json).
