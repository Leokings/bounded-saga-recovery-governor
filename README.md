...
Bradbury finalized a second byte-identical v0.2.1 deployment at `0xA2DDebc4CC8Eb21bb8eB45214Bfad1A4dE7A26Fd`. Three governance attempts for the first, underspecified evidence reference failed closed with timeout, no-majority, and disagree results; none wrote a decision. A separate explicit-evidence workflow then finalized all four setup writes unanimously and finalized governance with four `AGREE` votes and one `DETERMINISTIC_VIOLATION`. Latest-final reads show `DECIDED`, decision `1`, and `PLAN_SELECTED / UNIQUE_RECOVERY_MATCH / REFUND_CAPTURED_PAYMENT`, bound to the authenticated reports and digest-linked state. The proof retains both the negative attempts and the successful final workflow.
