...
`bradbury-2026-08-12-v0.2.1-deployment-and-smoke.json` is the current-source Bradbury record. It proves a finalized, five-vote-agree, byte-identical deployment at `0xA2DDebc4CC8Eb21bb8eB45214Bfad1A4dE7A26Fd`. It retains three failed-closed governance attempts for an underspecified evidence reference: timeout, no-majority, and finalized disagree, all with no decision write. A separate explicit-evidence reference then finalized four unanimous setup writes and governance with four `AGREE` and one `DETERMINISTIC_VIOLATION`. Latest-final state is `DECIDED`; decision `1` durably records `PLAN_SELECTED / UNIQUE_RECOVERY_MATCH / REFUND_CAPTURED_PAYMENT` and its source/report/request/decision digests. The record is submission-eligible positive Bradbury semantic evidence, but not unanimity or exact model-identity evidence.
